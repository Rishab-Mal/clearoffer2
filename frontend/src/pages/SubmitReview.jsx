      ...
      {steps.map((s, i) => (
        <div key={i} className="flex items-center gap-2">
          <div className={`flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold transition-all ${
            ...
            : i === current ? 'bg-amber-500 text-black ring-4 ring-amber-500/20'
          ...
          {i < steps.length - 1 && (
            <div className={`flex-1 h-0.5 w-8 ${i < current ? 'bg-amber-500' : 'bg-slate-200'}`} />
          )}
        </div>
      ))}
    </div>
...
export default function SubmitReview() {
  ...
  const setRating = (k, v) => set(k, v)

  const toggleTech = (t) => set('tech_used', form.tech_used.includes(t)
    ? form.tech_used.filter(x => x !== t)
    : [...form.tech_used, t])

  const toggleTopic = (t) => set('interview_topics', form.interview_topics.includes(t)
    ? form.interview_topics.filter(x => x !== t)
    : [...form.interview_topics, t])

  const validateStep = () => {
    const e = {}
    if (step === 0) {
      if (!form.company) e.company = 'Required'
      if (!form.role_title) e.role_title = 'Required'
    }
    if (step === 1) {
      if (form.work_description.length < 100) e.work_description = 'Minimum 100 characters'
      if (!form.rating_work) e.rating_work = 'Required'
    }
    if (step === 3) {
      if (!form.one_line_summary) e.one_line_summary = 'Required'
      if (form.would_return === null) e.would_return = 'Required'
    }
    setErrors(e)
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...form,
        internship_year: form.internship_year ? Number(form.internship_year) : null,
        duration_months: form.duration_months ? Number(form.duration_months) : null,
        interview_rounds: form.interview_rounds ? Number(form.interview_rounds) : null,
        interview_difficulty: form.interview_difficulty ? Number(form.interview_difficulty) : null,
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-10">
        <div className="mb-6">
          <Link to="/dashboard" className="flex items-center gap-2 text-slate-500 hover:text-slate-700 text-sm mb-4">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-8">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <select
                  ...
                  className={`w-full border rounded-xl px-4 py-3 text-sm outline-none ${errors.company ? 'border-red-400' : 'border-slate-200 focus:border-amber-400'}`}
                >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={setE('role_title')}
                  placeholder="e.g. Software Engineer Intern"
                  className={`w-full border rounded-xl px-4 py-3 text-sm outline-none ${errors.role_title ? 'border-red-400' : 'border-slate-200 focus:border-amber-400'}`}
                />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    value={form.team}
                    onChange={setE('team')}
                    placeholder="e.g. Billing, Ads, Growth"
                ...
                <div>
                  ...
                  <select
                    value={form.internship_year}
                    onChange={setE('internship_year')}
                    className="w-full border border-slate-200 focus:border-amber-400 rounded-xl px-4 py-3 text-sm outline-none"
                  >
          ...
          {step === 1 && (
            <div className="space-y-5">
              ...
              <div className="space-y-4 bg-slate-50 rounded-xl p-4">
                <RatingSlider label="Work quality" name="rating_work" value={form.rating_work} onChange={setRating} />
                <RatingSlider label="Mentorship" name="rating_mentorship" value={form.rating_mentorship} onChange={setRating} />
                <RatingSlider label="Compensation" name="rating_compensation" value={form.rating_compensation} onChange={setRating} />
                <RatingSlider label="Culture" name="rating_culture" value={form.rating_culture} onChange={setRating} />
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={setE('work_description')}
                  rows={5}
                  placeholder="What did you actually work on? Who was your manager like? What was the most meaningful part of your internship?"
                  className={`w-full border rounded-xl px-4 py-3 text-sm outline-none resize-none ${errors.work_description ? 'border-red-400' : 'border-slate-200 focus:border-amber-400'}`}
                />
                  ...
                  {TECH_OPTIONS.map(t => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                        form.tech_used.includes(t) ? 'bg-amber-500 text-black' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                      }`}
                    >
                      {t}
                    </button>
          ...
          {step === 2 && (
            <div className="space-y-5">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                  Difficulty
                  <span className="text-slate-400 font-normal ml-2">{['', 'Very Easy', 'Easy', 'Medium', 'Hard', 'Very Hard'][form.interview_difficulty]}</span>
                </label>
                ...
                <div className="flex flex-wrap gap-2">
                  {INTERVIEW_TOPICS.map(t => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                        form.interview_topics.includes(t) ? 'bg-amber-500 text-black' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                      }`}
                    >
                      {t}
                    </button>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={setE('specific_questions')}
                  rows={3}
                  placeholder="Specific questions or problem types — don't worry about exact wording."
            ...
            <div className="space-y-5">
                  ...
                  {[true, false].map(v => (
                    <button
                      ...
                      className={`py-3 rounded-xl text-sm font-semibold border-2 transition-colors ${
                        form.would_return === v
                          ? v ? 'bg-green-50 border-green-500 text-green-700' : 'bg-red-50 border-red-400 text-red-600'
              ...
              <div>
                ...
                <input
                  ...
                  onChange={setE('one_line_summary')}
                  placeholder="e.g. Best summer of my life. Real ownership, shipped to millions."
                  maxLength={120}
                  className={`w-full border rounded-xl px-4 py-3 text-sm outline-none ${errors.one_line_summary ? 'border-red-400' : 'border-slate-200 focus:border-amber-400'}`}
                />
              ...
              <div className="bg-slate-50 rounded-xl p-4 space-y-3">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Your name will never be shown</p>
                  </div>
                </label>
                {!form.anonymous && (
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={form.show_university}
                      onChange={e => set('show_university', e.target.checked)}
                      className="accent-amber-500 w-4 h-4"
                    />
                    <div>
                      <p className="text-sm font-semibold text-slate-700">Show my university</p>
                      <p className="text-xs text-slate-500">Helps others weigh your experience</p>
                    </div>
                  </label>
                )}
              </div>
          ...
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-slate-100">
            <button
              onClick={back}
              disabled={step === 0}
              className="flex items-center gap-1.5 text-sm font-medium text-slate-500 hover:text-slate-700 disabled:opacity-30 disabled:cursor-not-allowed"
            >
            ...
            {step < 3 ? (
              <button
                onClick={next}
                className="flex items-center gap-1.5 bg-amber-500 hover:bg-amber-400 text-black font-bold text-sm px-6 py-2.5 rounded-xl transition-colors"
              >
              ...
              <button
                ...
                disabled={submitting}
                className="flex items-center gap-1.5 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-black font-bold text-sm px-6 py-2.5 rounded-xl transition-colors"
              >
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
      <div className="bg-white border border-slate-200 rounded-2xl p-10 max-w-md w-full text-center shadow-sm">
        ...
        <p className="text-slate-500 mb-2">Your review unlocks full access and helps hundreds of students make better decisions.</p>
        <p className="text-xs text-slate-400 mb-6">Reviews are live after a quick moderation check (usually under 24 hours).</p>
        <div className="flex flex-col gap-3">
          <Link to="/search" className="bg-amber-500 hover:bg-amber-400 text-black font-bold py-3 rounded-xl text-sm transition-colors">
            Explore companies
          </Link>
          <Link to="/dashboard" className="text-slate-500 hover:text-slate-700 text-sm font-medium">
            Back to dashboard
          </Link>
        </div>
      </div>
